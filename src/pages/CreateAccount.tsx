...
export const CreateAccount: React.FC = () => {
  ...
  useEffect(() => {
    ...
    html.classList.add('auth-viewport-fix');
    body.classList.add('auth-viewport-fix');

    return () => {
      html.classList.remove('auth-viewport-fix');
      body.classList.remove('auth-viewport-fix');
    };
  }, []);
  ...
  if (isCheckingAuth) {
    return (
      <div className="min-h-screen min-h-[100dvh] bg-gradient-to-br from-[var(--bf-navy)] via-[var(--bf-navy-light)] to-[var(--bf-blue-primary)] flex items-center justify-center px-4 pt-[calc(env(safe-area-inset-top)+1rem)] pb-[calc(env(safe-area-inset-bottom)+1rem)]">
